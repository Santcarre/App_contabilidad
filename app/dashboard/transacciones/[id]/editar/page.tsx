...
export default function EditarTransaccionPage() {
  ...
  const { data: txData, isLoading: loadingTx } = useTransactions({ limit: 1000 });
  const { data: categories } = useCategories();
  const { data: sources } = useSources();
  const { data: ratesData } = useRates();
  const updateTransaction = useUpdateTransaction();

  const transaction = txData?.transactions.find((t) => t.id === id);
  ...
  useEffect(() => {
    if (transaction) {
      setFormData({
        ...
        amount: String(transaction.amountOriginal),
  ...
  if (loadingTx) {
    return (
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <Card><CardContent className="py-12 text-center">Cargando transacción...</CardContent></Card>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-2xl mx-auto space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold">Transacción no encontrada</h1>
          </div>
        </div>
        <Card>
          <CardContent className="py-12 text-center">
            <AlertCircle className="h-8 w-8 mx-auto mb-2 text-red-600" />
            <p className="text-muted-foreground">La transacción que buscas no existe o fue eliminada.</p>
            <Button className="mt-4" onClick={() => router.push("/dashboard/transacciones")}>
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Modifica los detalles de la transacción</p>
        </div>
      </div>
